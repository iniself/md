...
> 更多图标：[FontAwesome icons](https://fontawesome.com/search?ic=free-collection)。
...
- **图片**：和链接类似，只需在前面加上 `!`。Docs^red:+^ 支持定义图片长宽（`px` | `%`）和填充方式（`contain` | `cover` | `fill` ...）：
  ...
  `![nori 项目](assets/images/nori.png =200 @cover)`
...
Docs^red:+^ 支持 Admonition 语法。包括如下提示类型：
...
- **行内公式**：用 `$` 包裹公式，如 $E = mc^2$。行内公式可以嵌入文本样式语法中。比如字体更小：=::60% $E = mc^2$::=
- **块级公式**：用 `$$` 包裹公式，如：
...
💡 **还可以 1️⃣ 通过 slash menu 或则 `Command + Option + L` 使用 [MathLive](https://mathlive.io/) 便捷输入公式；2️⃣ 点击渲染好的公式在 MathLive 窗口里进行修改。**
...
**💡 分割符仅对不带其他格式的纯文字有效**。比如对粗体文字注音时分割符无效
...
![nori 项目](assets/images/nori.png)

![nori 项目](assets/images/nori.png =200)
...
![加我微信聊天](assets/images/auiwechat.png =30%)
