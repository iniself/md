...
// Vivliostyle
export function createPDFBody(options: PDFOptions, safeTitle: string, htmlStr: string, chatVarCss: string, printMargin: string, pdfLib: PdfLib, script: string = ``) {
  let topCenter = ``
  if (safeTitle) {
    topCenter = `
      @top-center {
  ...
  if (pdfLib === `vivliostyle`) {
    bodyDoc = `<!DOCTYPE html>${createTag(`html`, `
<html>
<head>
  <meta charset="utf-8">
...
export function createPDFScript(pdfLib: PdfLib, options: {
  isElectron: boolean
  isTauri: boolean
  title: string
}) {
  if (options.isTauri && pdfLib === `paged`) {
...
`)
  ...
  else if (options.isTauri && pdfLib === `vivliostyle`) {
    return createTag(`script`, `
window.onafterprint = () => window.close()
window.print()
setTimeout(() => {
...
`)
  ...
  if (pdfLib === `vivliostyle` && !options.isElectron && !options.isTauri) {
    return createTag(`script`, `
window.onafterprint = () => {
  window.top?.postMessage({ type: "PRINT_DONE" }, "*")
}
`)
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if(${options.isElectron}){
    window.dispatchEvent(new CustomEvent("paged-done"))
  }else{
    window.onafterprint = () => {
      if (window.opener) {
